...
export class TimelineService {
  ...
  /**
   * Move a timeline entry's draft one position up/down by swapping `order`
   * with its immediate neighbor — correct under pagination, unlike
   * `reorderEntries` which needs the full ordered id list.
   */
  static async moveOrder(
    ...
    return await db.$transaction(async (tx) => {
      ...
      const neighbor = await tx.timelineEntryVersion.findFirst({
        where: {
          state: "DRAFT",
      ...
      await tx.timelineEntryVersion.update({ where: { id: current.id }, data: { order: neighbor.order } });
      await tx.timelineEntryVersion.update({ where: { id: neighbor.id }, data: { order: current.order } });

      await recordAudit({
        action: "TIMELINE_REORDERED",
        entityType: "TimelineEntry",
        entityId: id,
        summary: `Moved a timeline entry ${direction} in the sequence.`,
        context: auditContext,
        tx,
      });
